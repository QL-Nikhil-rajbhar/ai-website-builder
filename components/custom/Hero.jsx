...
function Hero() {
    ...
    const onGenerate = async (input, skipQuestions = false) => {
        if (selectedImages.length > 0) {
            setUploading(true);

            try {
                const msg = { role: "user", content: input, skipQuestions };
                const urls = await uploadImagesAndGetUrls(selectedImages);

                setMessages([msg]);
                setUrls(urls);
                const workspaceID = await CreateWorkspace({ messages: [msg], urls: urls });
    ...
    return (
        <div className="min-h-screen bg-gray-950 relative overflow-hidden">
            ...
            <div className="absolute left-12 top-0 h-[500px] w-[1000px] -translate-x-1/2 bg-[radial-gradient(circle_400px_at_50%_300px,#3b82f625,transparent)]"></div>
            ...
            <div className="container mx-auto px-4 py-16 relative z-10">
                <div className="flex flex-col items-center justify-center space-y-12">
                    <div className="text-center space-y-6">
                        ...
                        <h1 className="text-6xl md:text-7xl font-bold text-transparent bg-clip-text bg-[linear-gradient(45deg,#60a5fa30,#ec4899)] leading-tight">
                    ...
                    <div className="w-full max-w-3xl bg-gray-900/40 backdrop-blur-2xl rounded-xl border-2 border-electric-blue-500/40 shadow-[0_0_40px_5px_rgba(59,130,246,0.15)]">
                        <div className="p-2 bg-gradient-to-r from-electric-blue-500/10 to-purple-500/10">
                            <div className="bg-gray-900/80 p-6 rounded-lg space-y-4">
                                {/* Textarea */}
                                <textarea
                                ...
                                <div className="space-y-2">
                                    <input
                                        ...
                                        className="bg-gray-700 text-white rounded-xl px-2 py-2 border border-gray-600 cursor-pointer text-sm w-full"
                                    />
                                ...
                                {userInput && (
                                    <div className="flex flex-col gap-2 pt-2">
                                        <button
                                            onClick={() => onGenerate(userInput, false)}
                                            disabled={isEnhancing || uploading}
                                        ...
                                        >
                                            ...
                                            <span className="ml-2 font-semibold">Generate with Questions</span>
                                        </button>

                                        <button
                                            onClick={() => onGenerate(userInput, true)}
                                            disabled={isEnhancing || uploading}
                                            className={`flex items-center justify-center bg-gradient-to-r from-green-500 to-teal-500 hover:from-green-600 hover:to-teal-600 rounded-xl px-4 py-3 transition-all duration-200 ${(isEnhancing || uploading) && 'opacity-70 cursor-not-allowed'}`}
                                        >
                                            <ArrowRight className="h-6 w-6" />
                                            <span className="ml-2 text-sm">Skip Questions - Generate Now</span>
                                        </button>
                                    </div>
                                )}
                                ...
                                {!userInput && (
                                    <div className="text-center text-gray-500 text-sm pt-2">
                                        Enter a prompt to see generation options
                                    </div>
                                )}
                            </div>
                        </div>
